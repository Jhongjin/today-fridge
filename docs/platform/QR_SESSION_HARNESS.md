...
After filling the packet, run `npm run qa:commander-review-packet:check -- <packet.md> --expected-commit <sha> --expected-actions-run-url <queue-preview-run-url> --expected-preview-url <https-preview-or-qr-url>` so stale packet metadata cannot approve the wrong Queue Preview commit, run, or target URL. The packet also requires the successful Queue Preview run URL in metadata, using the `https://github.com/<owner>/<repo>/actions/runs/<run-id>` format, and the preview or QR target must be an HTTPS URL. GitHub Actions can fill it from `GITHUB_REPOSITORY` and `GITHUB_RUN_ID`; local generation should pass `--actions-run-url`.
