...
Run `npm run qa:qr-session:index -- --help` to print the current index options. Use `--github-summary` when running the index in GitHub Actions.
...
Use `npm run qa:rating-evidence` to create the rating evidence packet that feeds that commander decision, then validate it with `npm run qa:rating-evidence:check`.
