...
For automation evidence, `npm run --silent check:bundle -- --json` prints total bytes, budget bytes, source map paths, and file sizes after `npm run build`. Queue Preview also runs `npm run check:bundle -- --github-summary` and `npm run qa:console-assets -- --github-summary` so the workflow summary records bundle budget and console asset evidence.
