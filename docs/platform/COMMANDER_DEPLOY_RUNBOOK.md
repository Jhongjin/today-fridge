...
The required local commands include both `npm run qa:console-assets` and `npm run qa:screenshots` so console upload assets, 360/390 first-fold evidence, and full-flow screenshot evidence are refreshed for the exact reviewed commit.
...
It requires `npm run external-rewards:check-prereqs`, so the packet records whether real external rewards were not requested, ready, or blocked by missing Toss console values.
